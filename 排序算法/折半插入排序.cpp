...
//折半插入排序
template<class ElemType>
void BinInsertSort(SqList<ElemType> &A)
{
    int i,j,low,high,mid;
    for(i=1; i<A.ListLength(); i++)
    {
        ElemType e=A.GetElem(i);
        low=0,high=i-1;
        while(low<=high)
        {
            mid=(low+high)/2;
...
int main()
{
    ...
    cout<<"元素随机生成的线性表建立完毕...开始进行折半插入排序"<<endl;
    start=clock();
    BinInsertSort(A);
    finish=clock();
    cout<<"折半插入排序完成"<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s"<<endl<<endl;
    ...
    cout<<"元素已经有序的线性表建立完毕...开始进行折半插入排序"<<endl;
    start=clock();
    BinInsertSort(B);
    finish=clock();
    cout<<"折半插入排序完成"<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s";
    return 0;
}
