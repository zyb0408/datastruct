...
int main()
{
    ...
    cout<<"元素随机生成的线性表建立完毕...开始进行冒泡排序"<<endl;
    start=clock();
    BubbleSort(A);
    finish=clock();
    cout<<"冒泡排序完成"<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s"<<endl<<endl;
    ...
    cout<<"元素已经有序的线性表建立完毕...开始进行冒泡排序"<<endl;
    start=clock();
    BubbleSort(B);
    finish=clock();
    cout<<"冒泡排序完成"<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s";
    return 0;
}
