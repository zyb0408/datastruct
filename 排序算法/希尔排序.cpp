...
//希尔排序（主函数）
template<class ElemType>
void ShellSort(SqList<ElemType> &A, int dlta[], int t) //t为希尔排序的趟数
{
    int i;
    for(i=0; i<t; i++)
    {
...
//希尔排序（一趟）
template<class ElemType>
void ShellInsert(SqList<ElemType> &A, int dk)
{
    int i,j;
    for(i=dk; i<A.ListLength(); i++)
    {
        if(A.GetElem(i)<A.GetElem(i-dk))
        {
            ElemType e=A.GetElem(i);
            for(j=i-dk; j>=0&&e<A.GetElem(j); j-=dk)
            {
                A.SetElem(j+dk,A.GetElem(j));
            }
            A.SetElem(j+dk,e);
        }
    }
