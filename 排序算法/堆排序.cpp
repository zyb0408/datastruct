...
//建堆
template<class ElemType>
void HeapSort(SqList<ElemType> &A)
{
    int i;
    ElemType e;
    for(i=A.ListLength()/2-1; i>=0; i--)
        HeapAdjust(A,i,A.ListLength());
    //A.ListTraverse();
    for(i=A.ListLength()-1; i>=0; i--)
    {
        e=A.GetElem(0);
        A.SetElem(0,A.GetElem(i));
        A.SetElem(i,e);
        HeapAdjust(A,0,i);
        //if(i!=0)
        //A.ListTraverse();
...
//堆调整（最大堆）
template<class ElemType>
void HeapAdjust(SqList<ElemType> &A, int hole, int size)
{
    ElemType e=A.GetElem(hole);
    int i;
    for(i=2*hole+1; i<size; i=i*2+1)
    {
        if(i<size-1&&A.GetElem(i)<A.GetElem(i+1))
            i++;
        if(e>=A.GetElem(i))
            break;
        A.SetElem(hole,A.GetElem(i));
        hole=i;
    }
    A.SetElem(hole,e);
}
...
int main()
{
    ...
    finish=clock();
    cout<<"线性表A的长度为 "<<A.ListLength()<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s"<<endl;
    cout<<"元素随机生成的线性表建立完毕...开始进行堆排序"<<endl;
    start=clock();
    HeapSort(A);
    finish=clock();
    cout<<"堆排序完成"<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s"<<endl<<endl;
    ...
    finish=clock();
    cout<<"线性表B的长度为 "<<B.ListLength()<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s"<<endl;
    cout<<"元素已经有序的线性表建立完毕...开始进行堆排序"<<endl;
    start=clock();
    HeapSort(B);
    finish=clock();
    cout<<"堆排序完成"<<"\t用时"<<(double)(finish-start)/CLOCKS_PER_SEC<<"s";
    return 0;
}
