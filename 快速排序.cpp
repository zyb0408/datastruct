...
//快速排序的实现（外壳）
template<class ElemType>
void QuickSort(SqList<ElemType> &A)
{
...
//快速排序的实现（递归）
template<class ElemType>
void QuickSort(SqList<ElemType> &A, int low, int high)
{

    if(low<high)
    {
        int k=divide(A,low,high);
        cout<<k<<" "<<A.GetElem(k)<<endl;
        A.ListTraverse();
        QuickSort(A,low,k-1);
...
//划分函数（快速排序）
template<class ElemType>
int divide(SqList<ElemType> &A, int low, int high)
{
    ElemType e=A.GetElem(low);
    while(low<high)
    {
        while(low<high&&A.GetElem(high)>=e)
            high--;
            ElemType p=A.GetElem(high);
            A.SetElem(high,A.GetElem(low));
            A.SetElem(low,p);
        while(low<high&&A.GetElem(low)<=e)
            low++;
            p=A.GetElem(high);
            A.SetElem(high,A.GetElem(low));
            A.SetElem(low,p);
