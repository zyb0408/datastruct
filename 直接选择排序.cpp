...
//直接选择排序
template<class ElemType>
void SimpleSelectSort(SqList<ElemType> &A)
{
    int i,j,n,k,spot;//k用来记录当前趟中最小的元素的位置
    ElemType e;
    cin>>n;
    for(i=1; i<=n; i++)
    {
    ...
    for(i=0; i<A.ListLength()-1; i++)//前面n-1趟都排好了，最后一趟不用比了
    {
        spot=0;
        k=i;
        for(j=i+1; j<A.ListLength(); j++)
        {
            if(A.GetElem(j)<A.GetElem(k))
            {
                spot=1;
                k=j;
            }
