...
public:
    ...
    //计算以元素值x为根结点的（子）二叉树的高度（递归）
    int GetBinaryTreeHeight_Cursive( BinaryTreeNode<ElemType> *root, ElemType &x, int &flag) const;
    //统计结点个数
...
//计算以元素值x为根结点的左、右（子）二叉树的高度（外壳部分，用户函数）
template<class ElemType>
int GetBinaryTreeHeight(BinaryTree<ElemType> &T,ElemType &x,int flag)
{
    if(T.GetRoot()==NULL||Location(T,x)==NULL)//1、树空；2、树中没有这个结点，则树高为0
        return 0;
    else
        return T.GetBinaryTreeHeight_Cursive(Location(T,x),x,flag);
}

//计算以元素值x为根结点的左、右（子）二叉树的高度（递归部分，成员函数）
template<class ElemType>
int BinaryTree<ElemType>::GetBinaryTreeHeight_Cursive(BinaryTreeNode<ElemType> *root,ElemType &x,int &flag) const
{
    int l,r;
    if(root==NULL)
        return 0;
...
int main()
{
    ...
    int flag;
    cin>>flag;
    cout<<GetBinaryTreeHeight(tree,x,flag);
    return 0;
}
